              ...
              <div className="relative z-10 shrink-0">
                 <div className={`w-24 h-24 md:w-32 md:h-32 rounded-full border-[4px] md:border-[6px] bg-surface-container-highest shadow-xl overflow-hidden ${community.creator?.plan?.toLowerCase() === 'elite' ? 'border-zinc-900' : 'border-surface'}`}>
                    {community.creator?.avatar_url ? (
                        <img src={community.creator.avatar_url} alt="Creator Avatar" className="w-full h-full object-cover" />
                    ) : (
                        <div className="w-full h-full flex items-center justify-center bg-primary text-white text-3xl md:text-5xl font-black uppercase">
                            {community.creator?.full_name?.charAt(0) || "C"}
                        </div>
                    )}
                 </div>
